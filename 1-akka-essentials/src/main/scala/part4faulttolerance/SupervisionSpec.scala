...
class SupervisionSpec extends TestKit(ActorSystem("SupervisionSpec"))
  ...
  "A supervisor" should {
    ...
    "terminates its child in case of a major error" in {
      val supervisor = system.actorOf(Props[Supervisor])
      supervisor ! Props[FussyWordCounter]
      val child = expectMsgType[ActorRef]
...
object SupervisionSpec {
  ...
  class Supervisor extends Actor {
    ...
    override val supervisorStrategy: SupervisorStrategy = OneForOneStrategy() { // only for failing actor
  ...
  class AllForOneSupervisor extends Supervisor {
    override val supervisorStrategy: SupervisorStrategy = AllForOneStrategy() { // apply for all the child actors regardless of one that is failing
      case _: NullPointerException => Restart
      case _: IllegalArgumentException => Stop
      case _: RuntimeException => Resume
      case _: Exception => Escalate
