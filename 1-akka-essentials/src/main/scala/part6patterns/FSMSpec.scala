...
class FSMSpec extends TestKit(ActorSystem("FSMSpec"))
  ...
  def runTestSuite(props: Props): Unit = {
    ...
    "deliver the product if I insert all tha money" in {
      val vendingMachine = system.actorOf(props)
      vendingMachine ! Initialize(Map("coke" -> 10), Map("coke" -> 3))
      vendingMachine ! RequestProduct("coke")
      expectMsg(Instruction("Please insert 3 dollars"))
...
object FSMSpec {
      ...
      case ReceiveMoney(amount) =>
        ...
        if (money + amount >= price) {
          ...
          context.become(waitForMoney(inventory, prices, product, // don't change
            ...
            startReceiveMoneyTimeoutSchedule, requester)) // I need to set the timeout again
        }
    }
  ...
  // step1 - define the states and the data of the actor
  trait VendingState
