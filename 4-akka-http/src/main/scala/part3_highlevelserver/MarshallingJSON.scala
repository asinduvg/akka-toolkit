...
import part3_highlevelserver.GameAreaMap.{AddPlayer, GetAllPlayers, GetPlayer, GetPlayersByClass, RemovePlayer}
    ...
    pathPrefix("api" / "player") {
      get {
        path("class" / Segment) { characterClass =>
          val playerByClassFuture = (rtjvmGameMap ? GetPlayersByClass(characterClass)).mapTo[List[Player]]
